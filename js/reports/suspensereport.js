...
function  buildQuery(){	
	...
	var query = " select distinct se.* from suspenseentry se, suspensedetails sed where se.date>='"+fromdate+"' and se.date<='"+todate+"'";
	...
	query = query + " and se.suspense_id = sed.suspense_id order by se.date,se.type";
//	alert(query);
	return query;
}
...
function buildQueryForProduct(){
	var productQuery = " select * from suspensedetails where  suspense_id=?";
