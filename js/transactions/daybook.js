...
function getAccounts(date){
	var params = "action=getAccountsByDate&date="+ date;
	sendAJAXRequest("/PSS/src/controller/DaybookController.php",params,"responseGetAccountsByDate");
}

function responseGetAccountsByDate(responseText, isSuccess){
	if(isSuccess == "true"){
...
function updatePanel(responseText){
	...
	if(accountsXML != null){
		...
		var noOfPayments =  accountsXML.getElementsByTagName("daybook").length;
		var htmlText = "";
		for(var i=0;i<noOfPayments;i++){
			...
			htmlText = htmlText + "<div id='" + txnId +"' class='panel-element' ondblClick=\"editAccounts('"+ txnId +"')\">";
				...
				htmlText = htmlText + "<div class='amountcell'>" + getAccountsAsString(pg,lpg,silver,cash) +  "</div>";
...
function getAccountsAsString(pg,lpg,silver,cash){
	var htmlText = "";
...
function prepareParamsToAddAccounts(){
	var params = null;
...
function addAccounts(){
	var isValid = validateAccountsForm();
	if(isValid){
		updateStatus("Processing . . .");
		var params = prepareParamsToAddAccounts();
		sendAJAXRequest("/PSS/src/controller/DaybookController.php",params,"responseAddAccounts");
	}
}
...
function responseAddAccounts(responseText,isSuccess){
	if(isSuccess == "true"){
		updateStatus("Account details added successfully");
		getClosing();
		clearForm();
		getAccountsBySelectedDate();
	}else{
		var errorMsg = responseText.replace("<error>","");
		var errorMsg = errorMsg.replace("</error>","");
		updateStatus(errorMsg);
...
function editAccounts(txnId){
	var params = "action=getAccountsById&txnid=" + txnId;
	sendAJAXRequest("/PSS/src/controller/DaybookController.php",params,"responseEditAccounts");
}

function responseEditAccounts(responseText,isSuccess){
	if(isSuccess == "true"){
		populateAccountsForm(responseText);
	}else{
		var errorMsg = responseText.replace("<error>","");
		var errorMsg = errorMsg.replace("</error>","");
		updateStatus(errorMsg);
...
function populateAccountsForm(responseText){
...
function deleteAccounts(){
	var isConfirmed = confirm("Are you sure to delete this daybook entry ?");
	if(isConfirmed){
		updateStatus("Processing . . .");
		var params = prepareParamsToDeleteAccounts();
		sendAJAXRequest("/PSS/src/controller/DaybookController.php",params,"responseDeleteAccounts");
	}
}

function responseDeleteAccounts(responseText,isSuccess){
	if(isSuccess == "true"){
		updateStatus("Daybook entry deleted successfully.");
		getClosing();
		clearForm();
		getAccountsBySelectedDate();
	}else{
...
function prepareParamsToDeleteAccounts(){
	var params = null;
	var date = getFormattedDate(document.getElementById("date").value,"/");
	var type = document.getElementById("type").value;
	var categoryid = document.getElementById("channelid").value;
...
function modifyAccounts(){
	...
	if(isValid){
		updateStatus("Processing . . .");
		var params = prepareParamsToModifyAccounts();
		sendAJAXRequest("/PSS/src/controller/DaybookController.php",params,"responseModifyAccounts");
	}
}

function responseModifyAccounts(responseText,isSuccess){
	if(isSuccess == "true"){
		updateStatus("Daybook entry updated successfully.");
		getClosing();
		clearForm();
		getAccountsBySelectedDate();
	}else{
...
function prepareParamsToModifyAccounts(){
	var params = null;
	var date = getFormattedDate(document.getElementById("date").value,"/");
	var type = document.getElementById("type").value;
	var categoryid = document.getElementById("channelid").value;
